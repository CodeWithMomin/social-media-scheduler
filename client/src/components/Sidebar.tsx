...
const Sidebar = ({isOpen,setIsOpen}:{isOpen: boolean,setIsOpen:(val:boolean)=> void }) => {
    ...
    const {logout,user}={
        ...
        user:{name:'Momin Zahoor',email:"mominzahoor11@gmail.com"}
    }
    const location=useLocation()
  return (
    <div className={`fixed inset-y-0 left-0 z-50 w-64 bg-white border-r
    border-slate-200 flex flex-col h-full transform transition-transform duration-200 
    ease-in-out md:relative md:translate-x-0 ${isOpen ? "translate-x-0":"-translate-x-full"}`}>

        {/* Logo */}
        <div className='p-6 pb-4'>
            <div className='text-xl tracking-tight text-slate-800 flex items-center
            gap-1.5'>
        ...
        <nav className='flex-1 px-3 space-y-1'>
            {navItems.map((item)=>{
                ...
                return (
                    ...
                    ${isActive? "bg-red-50 text-red-600 border-red-100":
        ...
        <div className='p-4 border-t border-slate-100'>
            ...
            className='mt-1 flex items-center gap-2 px-3 py-2 w-full
            rounded text-sm text-slate-500 hover:bg-red-50 hover:text-red-500
