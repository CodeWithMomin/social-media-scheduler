...
const AiComposer = () => {
  ...
  const [genrations, setGenrations] = useState<any[]>([])
  ...
  const fetchGenrations = async () => {
    setGenrations(dummyGenerationData)
  }
    ...
    <div className="max-w-4xl mx-auto space-y-12 pb-20 animate-in fade-in duration-700">
      ...
      <div className="space-y-6 text-center mb-20">
        ...
        <div className="relative group mt-12">
          <textarea className="w-full px-6 py-6 bg-white border-slate-300 rounded-xl text-slate-900
        placeholder-slate-400 outline-none focus:border-slate-400 transation resize-none h-40"
            placeholder="Share your idea... (e.g. A post about launch of our new eco-friendly coffee beans)"
          ...
          <div className="absolute bottom-4 right-2.5 flex items-center gap-3 text-sm">
            <button onClick={() => setGenerateImage(!generateImage)} className="flex items-center gap-3 bg-red-50 py-2 px-3 rounded-lg">
              <span>AI Image</span>
              <div className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full
              transition-colors duration-200 ease-in-out focus:outline-none 
            ${generateImage ? "bg-red-500" : "bg-slate-200"}`}>
                <span className={`pointer-events-none size-4 transform translate-y-0.5
                rounded-full bg-white ${generateImage ? "translate-x-4.5" : "translate-x-0.5"} `}></span>
              </div>
            </button>
        ...
        <div className="flex flex-wrap justify-center gap-2">
          {tones.map((t) => (
            <button key={t} onClick={() => setTone(t)} className={`px-4 py-1.5 rounded-full text-sm
                  transition-all border ${tone === t ? "bg-red-500 border-red-500 text-white" :
      ...
      <div className="space-y-6 pt-12 border-t border-slate-100">
        <div className="flex items-center justify-between text-slate-600">
          ...
          <span className="text-sm text-slate-500 bg-slate-50 px-2" >{genrations.length} Total</span>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {genrations.map((gen) => (
            <div key={gen._id} className="group bg-white rounded-2xl border border-slate-100 p-5
                    hover:bg-red-100 transition-all relative overflow-hidden">
              <div className="flex flex-col h-full space-y-4">
                <div className="flex items-center justify-between">
                  <span className="text-xs text-slate-400 uppercase
                          tracking-widest">{new Date(gen.createdAt).toLocaleString()}</span>
                ...
                <div className="flex items-center gap-2 pt-2">
                  <button onClick={() => setActiveSchedular(gen)} className="flex-1 bg-slate-100 hover:bg-red-500 hover:text-white
                          text-slate-600 text-xs py-2.5 rounded-lg transition-all">Schedule Post</button>
                </div>
              </div>
            </div>
          ))}
          {genrations.length === 0 && (
            <div className="col-span-full py-20 text-center space-y-2">
              ...
              >No content generated yet. Try generating some content using the AI.</p>
            </div>
          )}
        </div>
      </div>
      ...
      {activeSchedular && (
        <div className="fixed inset-0 min-h-screen z-50 flex items-center justify-center
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl
                        border border-slate-100 overflow-hidden flex flex-col max-h-[90vh]">

            <div className="flex items-center justify-between px-8 py-4 border-b 
                          border-slate-100 bg-slate-50/30">
              <h3 className="text-slate-900">Schedule Genrations</h3>
              <button onClick={() => setActiveSchedular(null)} className="p-2 rounded-full hover:bg-slate-100 text-slate-400
                            transition-colors">
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-4">
              ...
              <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100 space-y-4">
                <p className="text-slate-800 text-sm leading-relaxed whitespace-pre-wrap">
                  ...
                  {activeSchedular.mediaUrl && (
                    <img src={activeSchedular.mediaUrl} alt="preview"
                      className="w-full aspect-vedio object-cover rounded-xl border border-slate-200
                                  shadow-sm" />
                  )}
                </p>
              </div>
            ...
            <div className="p-8 bg-slate-50/50 border-t border-slate-50 space-y-8">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div className="relative">
                              ...
                              transition-all " value={scheduledData} onChange={(e)=>setScheduledDate(e.target.value)} />
                            </div>
                            <div className="relative">
                  ...
                  <button onClick={handleScheduled} className="w-full flex items-center 
                  justify-center gap-2 py-3 rounded-md bg-slate-200 text-sm 
                  text-slate-700 hover:bg-red-500 hover:text-white transition">
