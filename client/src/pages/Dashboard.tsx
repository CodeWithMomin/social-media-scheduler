import { ActivityIcon, CircleCheck, ClockIcon, SendIcon, Share2Icon, TrendingUpIcon } from "lucide-react"
...
const Dashboard = () => {
  ...
  useEffect(()=>{
    const fetchDashboardData=async ()=>{
      try {
        const [postRes,accountsRes,activityRes]=[{data:dummyPostsData},{data:dummyAccountsData},
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-slate-500 text-sm mt-0.5">
          Here's  what's happening with your social accounts toady.</p>
          ...
          {statCards.map((card)=>(
            ...
            border-slate-200 rounded-2xl p-5 hover:border-red-200 transition-all">
              <div className="flex items-center justify-between">
                ...
                <div className="text-xs absolute right-4 top-4 
                text-red-500 flex items-center gap-1">
                  <TrendingUpIcon className="size-3" />
                  {card.trend}
                </div>
              </div>
              <p className="text-sm text-slice-500 mt-1">{card.label}</p>
            </div>
          ))}
      </div>
      ...
      {activites.length === 0 ? (
        ...
        justify-center mb-3"><ActivityIcon className="size-6 text-slate-400"/></div>
        ...
        <div className="divide-y divide-slate-50">
          {activites.map((activity)=>(
            ...
            transition-colors">
              ...
              shrink-0 mt-0.5 bg-zinc-100 text-zinc-600">
