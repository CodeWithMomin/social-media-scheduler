...
const Scheduler = () => {
  ...
  useEffect(()=>{
   (async ()=> fetchPosts());
   const intervel=setInterval(async ()=> await fetchPosts() , 1000)
  ...
  const togglePlatform=(id:string)=>setSelectedPlatforms(
        ...
        <div className='bg-white rounded-2xl border border-slate-200 p-6'>
          ...
          <form onSubmit={handleSchedule} className='space-y-5'>
            ...
            <div>
              ...
              mb-2'>Platforms</label>
              <div className='flex flex-wrap gap-3'>
                {PLATFORMS.map((p)=>{
                    const active=selectedPlatforms.includes(p.id)
                  return (
                    <button onClick={()=>togglePlatform(p.id)} key={p.id} type='button' aria-label={p.name} aria-pressed={active} className={`flex items-center 
                      gap-1.5 p-3 rounded-md border transition-all
                    duration-150 ${active ? "bg-red-50 border-red-300 text-red-500 scale-105":
                         ...
                         {mediaFile ? (
                          ...
                          border-slate-200 bg-slate-50'>
                            ...
                            <button type='button' onClick={()=>setMediaFile(null)}
                             className='absolute top-2 right-2 size-7 bg-slate-900/60 hover:bg-slate-900/80 text-white rounded-full
                          ...
                          hover:border-red-300 hover:bg-red-50/30 transition-all group' >
                            ...
                            <input type="file" accept="image/*,video/*" className='hidden' onChange={(e)=>e.target.files?.[0] 
                         ...
                         <div className='grid grid-cols-1 sm:grid-cols-2 gap-3'>
                          ...
                          <div>
                            ...
                            <div className='relative'>
                              ...
                              <input type="date" required className='w-full pl-10 pr-4 py-2.5 bg-slate-50
                              border border-slate-200 rounded-lg text-slate-900 text-sm outline-none' 
                          ...
                          <div>
                            ...
                            <div className='relative'>
                              ...
                              text-slate-400 pointer-events-none'/>
                              <input type="time" required className='w-full pl-10 pr-4 py-2.5 bg-slate-50
                              border border-slate-200 rounded-lg text-slate-900 text-sm outline-none' 
            ...
            justify-center gap-2 py-3.5 bg-red-500 hover:bg-red-600 transition-all text-white
            rounded-lg'>
              {loading ? (
              <>
                ...
                {/* Upcoming Post */}
                  <div className='bg-white border rounded-2xl border-slate-200 overflow-hidden'>
                      <div className='flex items-center gap-2.5 px-5 py-4 border-b border-slate-100'>
                        ...
                        <span className='ml-auto text-xs font-bold bg-zinc-100 text-zinc-700
                        px-2 py-0.5 rounded-full'>{scheduled.length}</span>
                      </div>
                              ...
                              transition-colors'>
                                <div className='flex items-center justify-between mb-2'>
                                    ...
                                    <div className='flex items-center gap-2'>
                                      {post.mediaType && <span className='text-sm bg-slate-100
                ...
                {/* Published Post */}
                  <div className='bg-white border rounded-2xl border-slate-200 overflow-hidden'>
                      <div className='flex items-center gap-2.5 px-5 py-4 border-b border-slate-100'>
                        ...
                        <span className='ml-auto text-xs font-bold bg-zinc-100 text-zinc-700
                        px-2 py-0.5 rounded-full'>{published.length}</span>
                      </div>
                      <div className='max-h-72 overflow-y-auto divide-y divide-slate-50'>
                        {
                          published.length === 0 ?(
                              ...
                              transition-colors'>
                                <div className='flex items-center justify-between mb-2'>
                                    <div className='flex gap-1.5 items-center'>
                                      {post.platforms.map((pl:string)=>{
                                        const meta=PLATFORMS.find((p)=>p.id === pl);
                                        return meta ? <meta.icon className=" size-3.5
                                        text-slate-400"/>: null
                                      })}
                                    </div>
                                    <div className='flex items-center gap-2'>
